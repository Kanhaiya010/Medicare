...
const sanitizeReturnTo = (value, fallback = "/appointments") => {
  if (!value || typeof value !== "string") return fallback;
  const v = value.trim();
  if (!v.startsWith("/")) return fallback;
  if (v.startsWith("//")) return fallback;
  if (v.includes("://")) return fallback;
  if (v.startsWith("/doctor-admin") || v.startsWith("/admin")) return fallback;
  return v;
};
...
const buildFrontendBase = (req) => {
  ...
  if (referer) {
    try {
      ...
      // Ignore malformed referer; fallback below.
    }
  }
...
export const getAppointments = async (req, res) => {
  ...
  try {
    ...
    try {
      ...
      console.warn("getAppointments countDocuments failed:", countErr?.message);
    }
    return res.json({
      success: true,
      appointments: items,
    ...
    console.error("GetAppointments primary query error:", error);
    try {
      const rawItems = await Appointment.collection
        .find(filter)
        .sort({ createdAt: -1 })
        .skip(skip)
        .limit(limit)
        .toArray();
      const total = await Appointment.collection.countDocuments(filter);
      return res.json({
        success: true,
        appointments: rawItems,
        items: rawItems,
        meta: { page, limit, total, count: rawItems.length },
        warning: "Served via raw collection fallback",
      });
    } catch (fallbackErr) {
      console.error("GetAppointments fallback query error:", fallbackErr);
      // Return safe empty payload so admin page doesn't hard-fail.
      return res.json({
        success: true,
        appointments: [],
        items: [],
        meta: { page, limit, total: 0, count: 0 },
        warning: "Failed to query appointments",
      });
    }
  }
};
...
export const createAppointment = async (req, res) => {
  try {
    ...
    const existingBooking = await Appointment
      .findOne({
        doctorId,
    ...
    const returnTo = sanitizeReturnTo(returnToFromBody, "/appointments");
    const encodedReturnTo = encodeURIComponent(returnTo);
    const successUrl = `${frontBase}/appointment/success?session_id={CHECKOUT_SESSION_ID}&return_to=${encodedReturnTo}`;
    const cancelUrl = `${frontBase}/appointment/cancel?return_to=${encodedReturnTo}`;
