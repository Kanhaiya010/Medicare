...
function RescheduleButton({ appointment, onReschedule, disabled }) {
  ...
  const initialDate =
    appointment.date && !isDateBefore(appointment.date, todayISO)
      ? appointment.date
      : todayISO;
...
const ServiceAppointmentsPage = () => {
        ...
        .map((a) => {
          const timeStr =
            ...
            (a.hour !== undefined && a.minute !== undefined
              ...
              : a.rescheduledTo?.time || (a.slot && a.slot.time) || a.time || "");
          ...
          return {
            ...
            age: a.age || a.raw?.age || "",
            serviceName:
              a.serviceName || a.service || a.raw?.serviceName || (a.notes || "").slice(0, 40),
  ...
  async function changeStatusRemote(id, newStatus) {
    ...
    if (old.status === "Completed" || old.status === "Canceled") {
      pushToast("Cannot change status", `Appointment #${id} is already ${old.status}.`);
      return;
    }
          ...
          a.id === id
            ? {
                ...
                hour: parseTimeToParts(
                  updated.time || updated.rescheduledTo?.time || a.raw?.time ||
  ...
  async function rescheduleRemote(id, dateStr, time24) {
    ...
    setAppointments((prev) =>
      prev.map((a) =>
        a.id === id
          ? { ...a, date: dateStr, hour: hour12, minute: mm, ampm, status: "Rescheduled" }
          : a,
      ),
    );

    pushToast("Rescheduling", `Appointment #${id} → ${formatDateNice(dateStr)} ${timeStr}`);
    ...
    try {
      ...
      const finalTimeStr =
        updated.time || updated.rescheduledTo?.time || timeStr ||
      ...
      pushToast("Rescheduled", `Appointment #${id} moved to ${formatDateNice(finalDate)} ${finalTimeStr}`);
    } catch (err) {
      console.error("rescheduleRemote:", err);
      pushToast("Reschedule failed", err.message || "Failed to reschedule — reloading");
  ...
  async function cancelRemote(id) {
    ...
    try {
      ...
      setAppointments((prev) =>
        prev.map((a) =>
          a.id === id
            ? { ...a, status: updated.status || "Canceled", raw: updated || a.raw }
            : a,
        ),
      );
                    ...
                    <div className={serviceAppointmentsStyles.actionsContainer}>
                      <div className={serviceAppointmentsStyles.actionsInnerContainer}>
                        <div className="flex-1">
                          <RescheduleButton
                            key={`res-${a.id}-${a.date}-${a.hour}-${a.minute}-${a.ampm}`}
                            appointment={a}
