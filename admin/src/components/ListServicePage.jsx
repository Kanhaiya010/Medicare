...
export default function ListServicePage() {
  ...
  useEffect(() => {
    async function fetchServices() {
      try {
        ...
        const items =
          (body && (body.data || body.services || body.items)) || [];
        const normalized = items.map((svc) => ({
          id: svc._id || svc.id,
          name: svc.name,
          about: svc.about || "",
          instructions: svc.instructions || svc.preInstructions || [],
          instructionsText: (
            svc.instructions ||
            svc.preInstructions ||
            []
  ...
  async function startEdit(service) {
    ...
    if (service.id) {
      try {
        ...
        // ignore; fallback to existing service payload
      }
    }
    ...
    const normalized = {
      ...
      available: latest.available ?? latest.availability === "Available",
  ...
  function validateSlots(slots = [], nowMs = 0) {
  ...
  function updateSlot(slotId, field, value) {
    setEditForm((p) => {
      const _oldSlot = (p.slots || []).find((s) => s.id === slotId) || {};
      if (field === "date" && value) {
        if (value < todayISO) {
          addToast(
