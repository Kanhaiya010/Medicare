...
const AppointmentPage = () => {
  ...
  const loadServiceAppointments = useCallback(async () => {
    ...
    try {
      // Prefer createdBy query (works reliably in current backend setup)
      // and fallback to /me only when user id is not available.
      const endpoint = user?.id
        ? `/api/service-appointments?createdBy=${user.id}`
        : "/api/service-appointments/me";
      const resp = await API.get(endpoint, { headers });
      console.log(`Response from ${endpoint}:`, resp?.data);
      ...
      console.error(
        "Error loading service appointments:",
        err?.response?.data || err.message || err,
      );

      if (user?.id) {
        try {
          console.log("Retrying service appointments with ?createdBy=", user.id);
          const debugResp = await API.get(
            `/api/service-appointments?createdBy=${user.id}`,
            { headers },
          );
