...
const DoctorsPage = () => {
  ...
  const loadDoctors = async () => {
    ...
    try {
      ...
      setAllDoctors(normalizeDoctors(json));
      setError("");
    } catch (err) {
  ...
  return (
    <div className={doctorsPageStyles.mainContainer}>
              ...
              {displayedDoctors.length > 0 ? (
                displayedDoctors.map((doctor, index) => (
                  ...
                  >
                    ...
                    <div
                      className={`${doctorsPageStyles.experienceBadge} dp-exp-glow`}
                    >
                    ...
                    {doctor.available ? (
                      <Link
                        to={`/doctors/${doctor.id}`}
                        state={{ doctor: doctor.raw || doctor }}
                        className={`${doctorsPageStyles.bookButton} dp-glow-btn`}
                      >
                        <ChevronRight
                      ...
                      <button
                        type="button"
                        disabled
                        className={`${doctorsPageStyles.notAvailableButton} dp-disabled-btn`}
                      >
                        <MousePointer2Off
            ...
            {filteredDoctors.length > 8 && (
              <div className={doctorsPageStyles.showMoreContainer}>
                ...
                >
                  ...
                  <ChevronRight
                    className={`${doctorsPageStyles.showMoreIcon} ${showAll ? "rotate-90" : ""}`}
                  />
                </button>
              </div>
            )}
      ...
      <style>{doctorsPageStyles.customCSS}</style>
      <style>{`
        @keyframes fade-in {
